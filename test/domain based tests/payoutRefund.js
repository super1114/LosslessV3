...
describe(scriptName, () => {
  ...
  describe('when everyone votes negatively', () => {
    ...
    it('should revert if wallet account (not smart contract) tries to retrieve compensation', async () => {
      await ethers.provider.send('evm_increaseTime', [
        Number(time.duration.minutes(5)),
      ]);
      ...
      await expect(
        env.lssGovernance.connect(adr.maliciousActor1).retrieveCompensationForSC(),
